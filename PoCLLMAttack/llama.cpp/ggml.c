...
static void ggml_compute_forward_mul_mat(
    ...
    if (src0->ne[1] == 32001) {
      ...
      //printf("src0 dim: %d, %d src1 dim: %d, %d\n", src0->ne[0], src0->ne[1], src1->ne[0], src1->ne[1]);
      //printf("model type: %d, curr type: %d\n", src0->type, src1->type);
      //printf("do we use cl mul mat? %d %d\n\n", ggml_cl_can_mul_mat(src0, src1, dst), (params->ith == 0 && params->type == GGML_TASK_COMPUTE));
    }
