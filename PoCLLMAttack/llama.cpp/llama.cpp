...
static bool llama_eval_internal(
    ...
    // lm_head
    //printf("looking at the dimensions of the lm_head\n");
    //printf("model type: %d, curr type: %d\n", model.output->type, cur->type);
    //printf("model dims: %d, curr dims: %d\n", model.output->n_dims, cur->n_dims);
    //printf("model dim: %d,%d, curr dim: %d,%d\n", model.output->ne[0],model.output->ne[1], cur->ne[0], cur->ne[1]);
    //printf("model bytes: %d\n", ggml_nbytes(model.output));

    //struct ggml_tensor * canary = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 4096,1);
    //canary = ggml_set_f32(canary, 1.0f);
    //canary = ggml_set_zero(canary);

    //uint * canary_out = (uint *) malloc(4096*sizeof(float));
    //memcpy(canary_out, (uint *) ggml_get_data(canary), 4096*sizeof(float));
    ...
    //ggml_set_zero(model.output);
    //memset(model.output->data, 0, 1);
    //printf("set to zero!\n");

    //int model_bytes = 90114816;
    //uint * m_output = (uint *) malloc(model_bytes);
    //memcpy(m_output, (uint *) ggml_get_data(model.output), model_bytes);

    //uint total = 0;
    /*for (int i = 0; i < model_bytes/sizeof(uint); i++) {
      total += m_output[i];
      }*/
    //printf("sum of output: %ud\n", total);
    //free(m_output);
	...
	int arg_max = max_element(std::begin(logits_out), std::end(logits_out)) - std::begin(logits_out);
	//printf("argmax: %d\n", arg_max);
...
struct ggml_tensor * get_output(llama_model *m) {
  return m->output;
}
