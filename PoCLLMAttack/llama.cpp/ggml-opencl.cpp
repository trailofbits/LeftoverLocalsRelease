...
void dequantize_q5_0_single(__global const struct block_q5_0* x, const int ib, const int iqs, float* v0) {
    const float d = vload_half(0, &x[ib].d);
...
// First experiment which put in a canary (two .125f values in a row)
// followed by the value in y.
// This is essentially a trojan. The new version doesn't have to do this.
__kernel void ToB_trojan(__global struct block_q5_0* x, __local float* tmp, __global float* y, __global float* dst, const int ncols) {
...
// Mostly derived from existing kernel
__kernel void ToB_optimized_matvecmult(__global struct block_q5_0* x, __local float* tmp, __global float* y, __global float* dst, const int ncols) {
  ...
  // Cache vector
  for (int i = get_local_id(0); i < 4096; i += get_local_size(0)) {
    tmp[i] = y[i];
  }
  
  barrier(CLK_LOCAL_MEM_FENCE);
  for (int row = get_global_id(0); row < 32001; row += get_global_size(0)) {
    dst[row] = 0;
    for (int k = 0; k < 2048; k++) {
      int tid = k % 32;
      int i = (k / 32) * 2;
      const int col = i*32 + 2*tid;
      //const int col = i;
      const int ib = (row*4096 + col)/32;
      const int iqs = (col%32)/2;
      const int iybs = col - col%32;
      float v0, v1;
      dequantize_q5_0(x, ib, iqs, &v0, &v1);
      if (iybs + iqs < 4096) {
	dst[row] += v0 * tmp[iybs + iqs + 0];
      }
      if (iybs + iqs + 16 < 4096) {
	dst[row] += v1 * tmp[iybs + iqs + 16];
      }
    }
  }
}
...
static cl_kernel dequantize_mul_mat_vec_q4_0_cl, dequantize_mul_mat_vec_q4_1_cl, dequantize_mul_mat_vec_q5_0_cl, dequantize_mul_mat_vec_q5_1_cl, dequantize_mul_mat_vec_q8_0_cl, convert_mul_mat_vec_f16_cl, ToB_trojan_cl, ToB_optimized_matvecmult_cl;
...
void ggml_cl_init(void) {
    ...
    printf("ggml_opencl: selecting platform: '%s'\n", default_device->platform->name);
    printf("ggml_opencl: selecting device: '%s'\n", default_device->name);
    //exit(0);
    ...
    CL_CHECK((ToB_trojan_cl = clCreateKernel(program, "ToB_trojan", &err), err));
    CL_CHECK((ToB_optimized_matvecmult_cl = clCreateKernel(program, "ToB_optimized_matvecmult", &err), err));
...
static void ggml_cl_mul_mat_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            // compute
            cl_event ev_sgemm;
	    //printf("doing a clblast cl_float\n");
...
static void ggml_cl_mul_mat_q_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (src0->ne[1] == 32001) {
      //printf("dmmv type: %d\n", type);
      dmmv = &ToB_optimized_matvecmult_cl;
    }
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
                ...
                // compute
                size_t global = ne01 * CL_DMMV_BLOCK_SIZE;
                size_t local = CL_DMMV_BLOCK_SIZE;
		size_t local_mem_size = local;
		...
		if (src0->ne[1] == 32001) {
		  ...
		  global = 128 * local;
                ...
                CL_CHECK(clSetKernelArg(*dmmv, 1, sizeof(float) * local_mem_size, NULL));
                CL_CHECK(clSetKernelArg(*dmmv, 2, sizeof(cl_mem), &d_Y));
                CL_CHECK(clSetKernelArg(*dmmv, 3, sizeof(cl_mem), &d_D));
                CL_CHECK(clSetKernelArg(*dmmv, 4, sizeof(cl_int), &ncols));
		if (src0->ne[1] == 32001) {
		  //printf("right before we do the dmmv kernel %d\n", ncols);
		}
	    ...
	    if (src0->ne[1] == 32001) {
	      float m_value = 0.0;
	      int m_index = 0;
	      for (int zz = 0; zz < d_ne; zz++) {
		if (d[zz] > m_value) {
		  m_index = zz;
		  m_value = d[zz];
		}
	      }	    
	      //printf("argmax ggml-opencl: %d\n", m_index);
	    }
